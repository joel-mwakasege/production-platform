...
export const getScheduleHandler: RequestHandler = async (req, res): Promise<void> => {
  try {
    ...
    console.error('Error in getScheduleHandler:', err);
    res.status(500).json({ error: err.message || 'Failed to get schedule' });
  }
};
...
export const createShootDayHandler: RequestHandler = async (req, res): Promise<void> => {
  try {
    ...
    console.error('Error in createShootDayHandler:', err);
    res.status(500).json({ error: err.message || 'Failed to create shoot day' });
  }
};
...
export const assignSceneHandler: RequestHandler = async (req, res): Promise<void> => {
  try {
    ...
    console.error('Error in assignSceneHandler:', err);
    res.status(500).json({ error: err.message || 'Failed to assign scene' });
  }
};